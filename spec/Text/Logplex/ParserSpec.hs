...
spec = do
  describe "Logplex Parser" $ do
    ...
    it "should parse example heroku documents" $ do
      ...
      let entry = LogEntry "158" "1" (posixSecondsToUTCTime 1449986537) "host" "heroku" "router" "-" [] (Just "at=error code=H10 desc=\"App crashed\" method=POST path=\"/crash\" host=ancient-savannah-2923.herokuapp.com request_id=fdcf3bf3-97d3-48a5-8eff-3b28789e36ee fwd=\"120.148.233.212\" dyno= connect= service= status=503 bytes=\n")
